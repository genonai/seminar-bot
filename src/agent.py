...
TOOLS: list[dict[str, Any]] = [
    {
        ...
        "function": {
            "name": "set_topic",
            "description": (
                "**이미 다가올 일정의 slot_1 또는 slot_2 에 배정된 사람**의 토픽만 갱신. "
                "발표자 자체를 새로 일정에 넣는 게 아니라 *기존 배정의 토픽 컬럼만* 수정한다. "
                "target_presenter 가 호출자 본인이거나 null이면 본인 토픽. "
                "다른 사람 이름이면 운영자만 가능 (호출자 권한은 시스템 프롬프트에 명시됨). "
                "주의: 발표자가 아직 일정에 없으면 이 도구가 아니라 set_presenter(topic=...) 로 처리해야 함."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "set_presenter",
            "description": (
                "특정 회차의 slot_1 또는 slot_2 에 발표자 배정/교체/제거. **운영자 전용** — member/bystander 호출 금지. "
                "용도: (a) 빈 슬롯에 새 발표자 처음 배정, (b) 기존 발표자 다른 사람으로 교체, (c) 슬롯 비우기, "
                "(d) 자동 추첨 후 ad-hoc 으로 2명 발표 만들기. 이름과 토픽이 같이 주어지면 topic 파라미터에 한꺼번에 넣어 한 호출로 처리. "
                "예: '5/28 발표자 추가 황산하, graphRAG 소개' → 5/28 빈 슬롯에 배정 + 토픽까지 (slot은 비어있는 쪽). "
                "예: '5/28 한 명 더 추가, 김재선' → slot=2, name='김재선' (slot_1 이미 차있으므로). "
                "예: '6/4 두 번째 발표자 빼' → slot=2, name=null. "
                "예: '6/11 첫 발표자 임종석으로 바꿔, 토픽 RAG eval' → slot=1, name='임종석', topic='RAG eval'. "
                "주의: 발표자가 아직 일정에 없으면 set_topic 이 아니라 *이 도구* 를 써야 한다 (set_topic 은 이미 배정된 사람의 토픽 갱신 전용). "
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "slot": {
                        ...
                        "enum": [1, 2],
                        "description": (
                            "1=첫 번째 발표자, 2=두 번째 발표자. "
                            "사용자가 명시(예: '두 번째', '한 명 더') 안 했으면 시스템 프롬프트의 다가올 일정에서 *비어있는 슬롯* 을 골라라. "
                            "두 슬롯 다 비어있으면 1, 한쪽만 비면 그쪽, 둘 다 차있으면 운영자에게 확인 요청 (plan_next_steps)."
                        ),
                    },
                    "name": {
                        "type": ["string", "null"],
...
- *발표자 배정/교체/제거* (**운영자 전용**): 새 발표자 처음 배정, 빈 슬롯 채우기, 이름 바꾸기, 슬롯 비우기.
  → set_presenter(target_date='YYYY-MM-DD', slot=1|2, name='이름' 또는 null, topic='토픽' 또는 null)
   - member/bystander 가 이 종류 요청 하면 send_message 로 정중히 거절 ("발표자 변경은 운영자만 가능합니다").
   - 빈 슬롯에 새 사람 처음 배정도 *이 도구* 이지 set_topic 아님.
   - slot 결정 규칙 (사용자가 명시 안 한 경우, 다가올 일정 표 기준):
     · 양쪽 다 비어있음 → slot=1
     · 한쪽만 비어있음 → 비어있는 쪽
     · 양쪽 다 차있음 → plan_next_steps 로 운영자에게 어느 슬롯 교체할지 확인
   - 사용자 명시 키워드: '한 명 더', '추가', '두 번째' → slot=2. '첫', '메인' → slot=1.
   - '빼/제거/취소' → name=null.
   - 이름 + 토픽 동시 입력 ('5/28 발표자 추가 황산하, graphRAG 소개') → 한 번의 호출에 name과 topic 모두 넣어라.
   - 토픽 명시 안 하고 새 이름만 넣으면 기존 슬롯 토픽 자동 클리어 (정상 동작 — 앞 사람 토픽 인계 방지).
   - 사용자가 '5/28' 처럼 줄여 말하면 오늘 기준 가까운 미래 목요일로 보정해서 YYYY-MM-DD 만들어라.

# set_topic 대 set_presenter — tie-breaking
"이름 + 토픽" 패턴이 모호할 때:
  · 그 사람이 *다가올 일정 표(아래 # 다가올 일정)* 에 *이미 slot_1 또는 slot_2 로 명시돼 있으면* → set_topic
  · 표에 없거나 다른 회차에만 있고 사용자가 명시한 날짜의 슬롯에 없으면 → set_presenter(name=이름, topic=토픽)
  · 확신 안 서면 set_presenter 가 더 안전 (멱등하고 토픽도 같이 처리됨). admin 이 아니면 set_topic 만 가능.
